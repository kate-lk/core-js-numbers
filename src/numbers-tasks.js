...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cornerCos = scalarProduct / (vectorModule1 * vectorModule2);
  return Math.acos(cornerCos);
}
...
function roundToPowerOfTen(num, pow) {
  const num10 = num / 10 ** pow;
  return Math.round(num10) * 10 ** pow;
}
...
function isPowerOfTwo(num) {
  if (Number.isInteger(Math.log2(num) % 2)) {
    return true;
  }
  return false;
}
